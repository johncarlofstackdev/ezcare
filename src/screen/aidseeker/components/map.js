...
const Map = ({ eventInformation }) => {
    ...
    // const { aid_provider_location } = eventInformation;
    
    // const [aidProviderLoc, aidProviderLocSet] = useState({
    //     latitude: aid_provider_location.latitude,
    //     longitude:  aid_provider_location.longitude
    // });

    const [region, setRegion] = useState({});
    const { aid_provider_location, serviceType } = eventInformation;
    const aidProviderLoc = {
        latitude: aid_provider_location.latitude,
        longitude: aid_provider_location.longitude,
    };
    ...
    return (
        <MapView style={styles.map} provider={PROVIDER_GOOGLE} region={region}>
            ...
            <Marker coordinate={aidProviderLoc} image={MapIcon(serviceType).services} />

            <MapViewDirections
                origin={region}
                destination={aidProviderLoc}
                apikey={Google_API()}
                strokeWidth={5}
